...
export async function writeBenchmark(benches: Benchmark[], config: Config) {
    ...
    if (externalDataJsonPath) {
        ...
        // Only clone once for all benchmarks
        prevBench = await writeBenchmarkToGitHubPages(benches[0], config);
        for (let i = 1; i < benches.length; i++) {
            await writeBenchmarkToGitHubPagesWithRetry(benches[i], config, 10);
        }
    }

    // Handle comments/alerts for the first benchmark only to avoid spam
    if (prevBench !== null) {
        await handleComment(name, benches[0], prevBench, config);
        await handleAlert(name, benches[0], prevBench, config);
        await handleSummary(name, benches[0], prevBench, config);
    } else {
