...
async function validateOutputFilePath(filePath: string): Promise<string[]> {
    try {
        if (filePath.includes('*') || filePath.includes('?')) {
            // Handle glob pattern
            const files = await fg(filePath);
            if (files.length === 0) {
                throw new Error(`No files match the pattern '${filePath}'`);
            }
            return files;
        }
...
export async function configFromJobInput(): Promise<Config> {
    ...
    const outputFilePaths = await validateOutputFilePath(core.getInput('output-file-path'));
