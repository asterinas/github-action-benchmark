...
async function addBenchmarkToDataJson(benchName, bench, data, maxItems, summaryJsonPath) {
    var _a;
    const repoMetadata = getCurrentRepoMetadata();
    ...
    if (summaryJsonPath) {
        try {
            const summaryJson = await loadSummaryJson(summaryJsonPath);
...
async function writeBenchmarkToGitHubPagesWithRetry(bench, config, retry) {
    ...
    if (ghRepository) {
        ...
        try {
            await fs_1.promises.stat(benchmarkBaseDir);
    ...
    if (githubToken && autoPush) {
        try {
            ...
            if (retry > 0) {
                ...
                const maxRetries = (_c = config.retries) !== null && _c !== void 0 ? _c : 10; // Accessing potential retries property if added to config type later
                throw new Error(`Auto-push failed ${maxRetries} times since the remote branch ${ghPagesBranch} rejected pushing all the time. Last exception was: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    }
    else {
