...
function loadSummaryJson(jsonPath) {
    try {
        const content = fs_1.readFileSync(jsonPath, 'utf8');
        const json = JSON.parse(content);
        console.debug(`Loaded external JSON file at ${jsonPath}`);
        return json;
    } catch (err) {
...
function addBenchmarkToDataJson(benchName, bench, data, maxItems, summaryJsonPath) {
    const repoMetadata = getCurrentRepoMetadata();
    ...
    if (summaryJsonPath) {
        try {
            const summaryJson = loadSummaryJson(summaryJsonPath);
            const sortedEntries = sortEntriesBySummary(data.entries, summaryJson);
            data.entries = { ...sortedEntries };
            updateDisplayOnOverview(data.entries, benchName, summaryJson);
        } catch (err) {
...
function sortEntriesBySummary(entries, summaryJson) {
    const sortedEntries = {};
    summaryJson.benchmarks.forEach((name) => {
        if (entries[name]) {
            sortedEntries[name] = entries[name];
        }
    });
...
function updateDisplayOnOverview(entries, benchName, summaryJson) {
    const displayOnOverview = summaryJson.benchmarks.includes(benchName);
    const lastEntry = entries[benchName][entries[benchName].length - 1];
...
async function writeBenchmarkToGitHubPagesWithRetry(bench, config, retry) {
    var _a, _b;
    const { name, tool, ghPagesBranch, ghRepository, benchmarkDataDirPath, githubToken, autoPush, skipFetchGhPages, maxItemsInChart, summaryJsonPath, } = config;
